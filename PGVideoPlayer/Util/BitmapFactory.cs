...
namespace PGVideoPlayer.Util {
	public static class BitmapFactory {
		...
		public static WriteableBitmap ConverBitmapToWriteable(Bitmap bitmap, bool dispose = false) {
			return new WriteableBitmap(ConvertBitmapToSource(bitmap, dispose));
		}
		...
		public static WriteableBitmap WriteBitmapToWriteable(Bitmap bitmap, bool dispose = false) {
			var unlocked = BitmapDif.Unlock(bitmap);

			WriteableBitmap writeable = CreateWriteable(bitmap.Width, bitmap.Height, PixelFormats.Bgra32);
			writeable.WritePixels(
				new Int32Rect(0, 0, bitmap.Width, bitmap.Height),
				unlocked.Data.Scan0,
				unlocked.Data.Height * unlocked.Data.Stride,
				unlocked.Data.Stride);

			BitmapDif.Lock(unlocked, dispose);
